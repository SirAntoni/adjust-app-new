...
const cambiar_titulo_uno = function () {
  $("#formCambiarTituloUno").submit(function (e) {
    e.preventDefault();
    const data = $(this).serialize();
    $.ajax({
      url: './controller/web.php',
      ...
      success: function (response) {
        ...
        if (data.status == "success") {
          ...
          $("#formCambiarTituloUno").trigger('reset');
          $("#modalCambiarTituloUno").modal("hide");
          cargar_web();
...
const cambiar_titulo_dos = function () {
  $("#formCambiarTituloDos").submit(function (e) {
    e.preventDefault();
    const data = $(this).serialize();
    $.ajax({
      url: './controller/web.php',
      ...
      success: function (response) {
        ...
        if (data.status == "success") {
          ...
          $("#formCambiarTituloDos").trigger('reset');
          $("#modalCambiarTituloDos").modal("hide");
          cargar_web();
...
const hide_modules = function () {
  $("#formHideModules").submit(function (e) {
    e.preventDefault();
    const data = $(this).serialize();
    $.ajax({
      url: './controller/web.php',
...
var guardar_nosotrosImg = function () {
  ...
  $("#formNosotrosImg").submit(function (e) {
    e.preventDefault();
    const inputFile = document.getElementById('nosotrosImg');
    const maxSizeMB = 100; // Tamaño máximo permitido en MB (ajusta este valor)
    ...
    if (file.size > maxSizeBytes) {
      Swal.fire({
        title: 'Error!',
        text: `El archivo es demasiado grande. Tamaño máximo permitido: ${maxSizeMB}MB`,
        icon: 'error',
        confirmButtonText: 'Ok'
      })
      return;
...
const cargar_web = function () {
  ...
  Your browser does not support the video tag.
      ...
      $(".tituloUno").text(data.titulo_uno);
      $(".tituloUno").val(data.titulo_uno);
      $(".tituloDos").text(data.titulo_dos);
      $(".tituloDos").val(data.titulo_dos);
      $("#hide_mision").val(data.hide_mision);
      $("#hide_vision").val(data.hide_vision);
    }
  })
}
