...
class Frontend extends Conectar
{
    ...
    public function mostrar_autoparte($autoparte)
    {
        ...
        $sql = "SELECT uuid,autoparte,stock,color_uuid, tipo,detalles,descgeneral FROM autopartes WHERE uuid = ? and estado = 1";
    ...
    public function obtener_negocio($negocio){
        $sql = "SELECT * FROM negocios WHERE razon_social = ?";
        $sql = $this->db->prepare($sql);
        $sql->bindValue(1, $negocio);
        $sql->execute();
