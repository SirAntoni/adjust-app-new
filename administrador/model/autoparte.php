...
class Autopartes extends Conectar
{
    ...
    {
        if($padre_id === ''){
            $sql = "SELECT id,uuid,autoparte,stock,cover,color_uuid,categoria_uuid,tipo,detalles,descgeneral FROM autopartes WHERE categoria_uuid = ? AND padre_id IS NULL AND estado = 1";
            $sql = $this->db->prepare($sql);
            $sql->bindValue(1,$categoria);
            $sql->execute();
            return $sql->fetchAll(PDO::FETCH_ASSOC);
        }else{
            $sql = "SELECT id,uuid,autoparte,stock,cover,color_uuid,categoria_uuid,tipo,detalles,descgeneral   FROM autopartes WHERE padre_id = ? AND estado = 1";
            $sql = $this->db->prepare($sql);
    ...
    public function editar_autoparte($id,$autoparte,$stock,$cover,$color,$tipo,$detalles,$descgeneral)
    {
        if (empty($autoparte) || empty($stock) || empty($cover) || empty($tipo)) {
            ...
            $sql = "UPDATE autopartes SET autoparte = ?, stock = ?, cover = ?, color_uuid = ?, tipo = ?, detalles = ?, descgeneral = ? WHERE id = ?";
            $sql = $this->db->prepare($sql);
            ...
            $sql->bindValue(6, $detalles);
            $sql->bindValue(7, $descgeneral);
            $sql->bindValue(8, $id);
            $sql->execute();
